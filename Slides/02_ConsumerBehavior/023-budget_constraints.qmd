...
```{r budget-line-graph, fig.align='center'}
# Install and load ggplot2 if not already installed
# install.packages("ggplot2")
library(ggplot2)

# Create data for the budget line
budget_data <- data.frame(x = c(0, 10), y = c(10, 0))

# Create data for the points at intersections
points_data <- data.frame(x = c(0, 10), y = c(10, 0), label = c("M/Py", "M/Px"))

# Plot the budget line and add points
ggplot(budget_data, aes(x, y)) +
  geom_line(color = "blue", lwd = 1.5) +
  geom_hline(yintercept = 0, linetype = "solid", color = "black", lwd = 2) + 
  geom_vline(xintercept = 0, linetype = "solid", color = "black", lwd = 2) + 
  geom_point(data = points_data, aes(x, y), color = "red", size = 5) +
  geom_text(data = points_data, aes(x, y, label = label), vjust = 1.5, hjust = 1.2, color = "red") +
  # Customize axes labels and plot title
  labs(x = "Quantity of X", y = "Quantity of Y") +
  theme_minimal()
```
...
- The [MRS]{.hi-teal} represents how much $y$ you [would be willing to]{.hi-teal} give up in order to get a unit of $x$

. . .

- The [Price Ratio]{.hp} represents how much $y$ you [would have to]{.hp} give up in order to get a unit of $x$
...
[Back to the board]{.note .realbig .vertical-center}
